...
def _rule_hints(legacy: dict[str, Any], modern: dict[str, Any]) -> list[str]:
    ...
    plan_charges_agree = (
        abs(Decimal(str(legacy["plan_charge"])) - Decimal(str(modern["plan_charge"]))) <= CENT
    )
...
def compare(period: str, meridian_dir: Path, build: bool = True, limit: int = 25) -> int:
    ...
    print()
    print(
        f"{'REF':<9} {'CUSTOMER':<26} {'PRV':<4} "
        f"{'MERIDIAN':>12} {'VANTAGE':>12} {'DELTA':>11}  RULES"
    )
    print("-" * 104)
    for row in rows[:limit]:
        ref, customer, province, left_total, right_total, delta, hint_text = row
        print(
            f"{ref:<9} {customer:<26} {province:<4} "
            f"{left_total:>12.2f} {right_total:>12.2f} {delta:>11.2f}  {hint_text}"
        )
    if len(rows) > limit:
    ...
    if by_rule:
        print(
            "by rule:      "
            + "  ".join(f"{rule}=${amount:,.2f}" for rule, amount in sorted(by_rule.items()))
        )
    if by_province:
        print(
            "by province:  "
            + "  ".join(f"{prov}=${amount:,.2f}" for prov, amount in sorted(by_province.items()))
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-build", action="store_true", help="use the meridian binaries as they are"
    )
